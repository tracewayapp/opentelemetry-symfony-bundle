                ...
                ->arrayNode('doctrine')
                    ...
                    ->children()
                        ...
                        ->booleanNode('only_with_parent')
                            ->info('Only create DB spans when an active parent span exists. Suppresses orphan root spans from untraced contexts, e.g. the doctrine Messenger transport poll loop (BEGIN/SELECT/COMMIT every idle poll). Mirrors requireParentSpan from OTel JS instrumentations. Set false to restore pre-3.4 behavior (DB spans even without an active parent).')
                            ->defaultTrue()
                        ->end()
                    ->end()
                ->end()
