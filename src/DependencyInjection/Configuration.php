                ...
                ->arrayNode('doctrine')
                    ...
                    ->children()
                        ...
                        ->booleanNode('only_with_parent')
                            ->info('Only create DB spans when an active parent span exists. Suppresses orphan root spans from untraced contexts, e.g. the doctrine Messenger transport poll loop (BEGIN/SELECT/COMMIT every idle poll). Mirrors requireParentSpan from OTel JS instrumentations. Off by default for backward compatibility.')
                            ->defaultFalse()
                        ->end()
                    ->end()
                ->end()
