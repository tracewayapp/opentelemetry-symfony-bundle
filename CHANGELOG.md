...
- **`metrics.flush`** — exports accumulated measurements from the application lifecycle, which is what makes metrics work at all in worker runtimes (FrankenPHP, RoadRunner, Swoole). Spans and log records are pushed by their batch processors, which reconsider their schedule inside `BatchSpanProcessor::onEnd()` and `BatchLogRecordProcessor::onEmit()` on every item produced; a metric data point is an update to a running aggregation, not an event that ends, so the SDK collects it only on `MeterProvider::forceFlush()` or `::shutdown()` — and the only trigger it registers is a shutdown hook. Per-request runtimes reach that hook constantly and never notice. A worker reaches it when it is recycled, so until then the application emits traces and logs normally while its metrics stay in memory, with no error anywhere to explain it. `MetricFlusher` now exports on `kernel.terminate`, `console.terminate` and `WorkerRunningEvent` — the last because a Messenger consumer runs for hours and `console.terminate` fires only when it stops. Enabled by default whenever `metrics.enabled` is true, and free on runtimes that do not need it: the first flush in a process is skipped, so a process that ends there — every PHP-FPM request, every one-shot command — is still served by the shutdown hook alone, with no second copy of the same cumulative state. Only a process that reaches a second flush is one the hook will not serve in time, which needs no detection of the runtime in use.
- **`metrics.flush.interval`** (unset by default, following `OTEL_METRIC_EXPORT_INTERVAL` and the SDK's default for it, 60 s) — the minimum spacing between exports, and the reason the flush is safe to enable. Each export carries the whole cumulative state, so its size follows the number of attribute combinations a worker has accumulated rather than the number of requests since the last export: flushing per request multiplies volume without carrying more information and can fill a collector's queue until it drops data. Skipping an export costs nothing, since cumulative instruments carry their totals into the next one. `0` exports on every hook.

- **`traceway:doctor` checks metric temporality against the producer model** — cumulative temporality assumes one long-lived owner per time series, which is the opposite of what PHP provides: a process per request under FPM, several independent workers side by side otherwise. Without `service.instance.id` those producers share a series identity and overwrite each other's totals, and nothing fails while the numbers stop meaning anything. The new `metrics_temporality` check warns on that combination and names both remedies, since which one applies depends on a runtime a console command cannot observe: give each worker an identity, or switch to delta temporality, where summing across producers is the intended semantics and no identity is needed. Identity is accepted from either source the SDK reads it from — `service_instance` in `OTEL_PHP_DETECTORS`, or `service.instance.id` in `OTEL_RESOURCE_ATTRIBUTES`, which is how an orchestrator injects a pod or task name and how this bundle passes `sdk.resource_attributes` on. `lowmemory` temporality is recognised as its own case rather than treated as cumulative: the PHP exporter maps it to no preference, leaving every synchronous instrument — all this bundle records — on delta.
