        ...
        $node
            ...
            ->children()
                ...
                ->integerNode('record_exception_min_status')
                    ->info('Skip recordException() for HttpExceptionInterface exceptions whose status code is below this value (e.g. 500 drops the exception event for 404 bot probes). Non-HTTP exceptions are always recorded; span status is unaffected. 0 (default) records every exception.')
                    ->defaultValue(0)
                    ->validate()
                        ->ifTrue(static fn (int $v): bool => 0 !== $v && ($v < 100 || $v > 599))
                        ->thenInvalid('record_exception_min_status must be 0 (record everything) or an HTTP status code between 100 and 599, got %s.')
                    ->end()
                ->end()
                ->arrayNode('console')
                    ...
                    ->children()
                        ->arrayNode('excluded_commands')
                            ->info('Console command names to exclude from tracing (e.g. cache:clear, assets:install). Always merged with the built-in long-running list (messenger:consume, messenger:consume-messages), so setting this key extends the defaults instead of replacing them; pre-3.5.0 it replaced them, silently re-enabling worker tracing. Use trace_long_running_commands to trace those too.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                        ->end()
                        ->booleanNode('trace_long_running_commands')
                            ->info('Trace the built-in long-running worker commands (messenger:consume, messenger:consume-messages) instead of excluding them. Their console span stays open for the whole process, so every message and poll query lands in one unbounded trace; per-message tracing is handled by the Messenger middleware.')
                            ->defaultFalse()
                        ->end()
                    ->end()
                ->end()
    ...
    private function buildMetricsNode(): ArrayNodeDefinition
    {
        ...
        $node
            ...
            ->children()
                ...
                ->end()
                ->arrayNode('flush')
                    ->info('Export accumulated measurements from the application lifecycle: kernel and console termination, and every iteration of a Messenger worker loop. Required in worker runtimes (FrankenPHP, RoadRunner, Swoole), where the SDK shutdown hook that normally exports metrics is only reached when the worker is recycled. Costs nothing under PHP-FPM: the first flush in a process is skipped, leaving the export to the shutdown hook exactly as before.')
                    ->canBeDisabled()
                    ->children()
                        ->floatNode('interval')
                            ->info('Minimum seconds between exports. Every export ships the whole cumulative state, so a shorter interval multiplies volume without carrying more information; skipping one loses nothing, since the next export carries the totals. 0 exports on every hook. Left unset it follows OTEL_METRIC_EXPORT_INTERVAL, or the SDK\'s default for it — the cadence a periodic reader would have used.')
                            ->defaultNull()
                            ->min(0)
                        ->end()
                    ->end()
                ...
                ->arrayNode('export')
                    ...
                    ->children()
                        ...
                        ->arrayNode('excluded_channels')
                            ->info('Monolog channel names to drop instead of exporting (e.g. deprecation, php). Useful to keep framework diagnostics out of your log storage; monolog-bundle\'s own "channels" key on the opentelemetry handler offers the same for inclusive/exclusive rules.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                        ->end()
                        ->arrayNode('excluded_http_codes')
                            ->info('HTTP status codes to exclude from log export: records whose context exception is an HttpExceptionInterface with one of these codes (e.g. 404 bot probes) are dropped instead of exported. Mirrors monolog-bundle\'s excluded_http_codes.')
                            ->integerPrototype()
                                ->min(100)
                                ->max(599)
                            ->end()
